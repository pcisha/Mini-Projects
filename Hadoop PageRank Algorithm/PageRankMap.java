	...
	public void map(LongWritable key, Text value, Context context)
		throws IOException, InterruptedException {
			...
			if (rrd.targetUrlsList.size()<=0){
				...
				int turl_size=rrd.targetUrlsList.size();
				double turl_val=rrd.rankValue/(double)turl_size;
				for(int i=0;i<turl_size;i++){
					sb.append("#"+rrd.targetUrlsList.get(i));
					context.write(new LongWritable(rrd.targetUrlsList.get(i)),new Text(turl_val+""));
				}
			} //for
			if(sb.length()>0){
